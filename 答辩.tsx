...
export async function apply(ctx: Context, config: Config) {
    ...
    ctx.command('galwife','娶二次元老婆').alias('/galwife').action(async ({ session }) => {
        ...
        //随机祝福
        let random_blessing = blessing[Math.floor(Math.random() * blessing.length)];
        ...
        let wife_blessing = ""
        let game_Name = ""
        if (config.标明出处) {let game_name = `「${random_game}」`}
        else {let game_name = ""}

        if (config.随机祝福) {let wife_blessing = `${random_blessing}`}
        else {let wife_blessing = ""}
        ...
        if (config.每日轮换) {
          ...
          await session.send(`<at id={session.userId} />あなたの嫁さんは${game_name}の「${random_name}」ございます。${wife_blessing}`);
          await session.send(h.image(imagePath));
          }
          else {if (user_data?.[0]?.user_time == user_Time) {
            ...
            if (config.标明出处) {let game_Name = `「${wife_Game}」の`}
            else {let game_Name = ""}

            await session.send( `<at id={session.userId} />スケベ!奥さんがいたのに！奥さんは${game_Name}「${user_Wife}」、次は許さないぞ！`);
            await session.send(h.image(user_imagePath));}else{
          ...
          await ctx.database.set("galgamewife", { user_id: user_id }, { user_time: user_Time, user_wife: random_name, wife_game:random_game});
            ...
            if (config.标明出处) {game_Name = `「${wife_Game}」の`}
            else {game_Name = ""}
            await session.send( `<at id={session.userId} />あなたの嫁さんは${game_Name}「${user_Wife}」ございます。${wife_blessing}`);
