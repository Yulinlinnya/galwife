...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        标明出处: koishi_1.Schema.boolean().default(false).description('※传播造成不良后果，那种事不要啊(;´༎ຶД༎ຶ`)'),
        随机祝福: koishi_1.Schema.boolean().default(true).description('※随机一句祝福语句，开宴席能带咱吗(╹ڡ╹ )'),
        概率计算: koishi_1.Schema.boolean().default(false).description('※开启为按游戏数量计，关闭为按老婆数量计，很方便呢（。＾▽＾）'),
        输出模式: koishi_1.Schema.boolean().default(true).description('※开启为图文混合，关闭为分开发送。很实用呢（＾∀＾●）ﾉｼ'),
    }).description('普通功能'),
    koishi_1.Schema.object({
        每日轮换: koishi_1.Schema.boolean().default(true).description('※记录每日与她的邂逅(✿◠‿◠)'),
    }).description('扩展功能'),
    koishi_1.Schema.object({
        自定义库: koishi_1.Schema.boolean().default(false).description('**☢不会使用请勿开启☢** 允许添加自己的老婆(○｀ 3′○)'),
        图片目录: koishi_1.Schema.string().description('**※文件夹的路径※** 向好友分享你自己的老婆库(/▽＼)'),
        图片JSON: koishi_1.Schema.string().description('**※JSON文件的路径※** 记录你和你老婆的相遇(★ ω ★)'),
    }).description('高级功能'),
]);
    ...
    ctx.command('galwife', '娶二次元老婆').alias('/galwife').action(async ({ session }) => {
        ...
        if (config.每日轮换) {
            ...
            if (user_data.length == 0) {
                ...
                if (config.输出模式) {
                    session.send(koishi_1.h.image(getImagepath(random_name)) + `<at id={session.userId} />あなたの嫁さんは${game_name}「${random_name}」ございます。${random_blessing}`);
                }
                else {
                    await session.send(`<at id={session.userId} />あなたの嫁さんは${game_name}「${random_name}」ございます。${random_blessing}`);
                    await session.send(koishi_1.h.image(getImagepath(random_name)));
                }
            }
            else {
                //用户存在，判断上传时间
                if (user_data?.[0]?.user_time == user_Time) {
                    ...
                    if (config.输出模式) {
                        await session.send(koishi_1.h.image(getImagepath(user_Wife)) + `<at id={session.userId} />スケベ！奥さんがいたのに！奥さんは${wife_Game}「${user_Wife}」、次は許さないわ！`);
                    }
                    else {
                        await session.send(`<at id={session.userId} />スケベ！奥さんがいたのに！奥さんは${wife_Game}「${user_Wife}」、次は許さないわ！`);
                        await session.send(koishi_1.h.image(getImagepath(user_Wife)));
                    }
                }
                else {
                    //时间错误，重新选择
                    ...
                    const wife_Game = config.标明出处 ? "「" + (await ctx.database.get("galgamewife", { user_id: user_id }))[0]?.wife_game + "」の" : "";
                    if (config.输出模式) {
                        await session.send(koishi_1.h.image(getImagepath(user_Wife)) + `<at id={session.userId} />あなたの嫁さんは${wife_Game}「${user_Wife}」ございます。${random_blessing}`);
                    }
                    else {
            ...
            if (config.输出模式) {
                await session.send(koishi_1.h.image(getImagepath(random_name)) + `<at id={session.userId} />あなたの嫁さんは${game_name}「${random_name}」ございます。${random_blessing}`);
            }
            else {
                await session.send(`<at id={session.userId} />あなたの嫁さんは${game_name}「${random_name}」ございます。${random_blessing}`);
                await session.send(koishi_1.h.image(getImagepath(random_name)));
