...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        标明出处: koishi_1.Schema.boolean().default(false).description('※传播造成不良后果本人概不负责'),
        随机祝福: koishi_1.Schema.boolean().default(true).description('※随机一句祝福语句'),
    }).description('普通功能'),
    koishi_1.Schema.object({
        每日轮换: koishi_1.Schema.boolean().default(true).description('※绑定用户和老婆每日刷新'),
    }).description('扩展功能'),
]);
    ...
    ctx.command('galwife', '娶二次元老婆').alias('/galwife').action(async ({ session }) => {
        ...
        if (config.每日轮换) {
            ...
            if (user_data.length == 0) {
                ...
                await session.send(koishi_1.h.image(imagePath));
            }
            else {
                //用户存在，判断上传时间
                if (user_data?.[0]?.user_time == user_Time) {
                    ...
                    await session.send(koishi_1.h.image(user_imagePath));
                }
                else {
                    //时间错误，重新选择
